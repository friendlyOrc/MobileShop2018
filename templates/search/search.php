...
<?php
    ...
    $page_nav .= '<li class="page-item"><a class="page-link"href="index.php?page_layout=search&key='.$keyword.'&page='.$pre_nav.'">Trang trước</a></li>';
        if($total <= 3){
            for($i = 1; $i <= $total; $i++){
    ...
    $page_nav .= '<li class="page-item"><a class="page-link" href="index.php?page_layout=search&key='.$keyword.'&page='.$next_nav.'">Trang sau</a></li>';
?>

<!--	List Product	-->
<div class="product-list row">
    <div id="search-result" class="col-md-12">Kết quả tìm kiếm với sản phẩm <span><?php echo htmlspecialchars($keyword, ENT_QUOTES, 'UTF-8'); ?></span></div>
    <?php
    ...
    if($count > 0){
        ...
        echo '<div class="col-md-4" style="padding: 10px 0px;">
                <div class="product-item text-center" style="background-color: white; border-radius: 10px;">
                    ...
                    <p>Giá Bán: <span>'.$row['prd_price'].'đ</span></p>
                </div>
            </div>';
        }
